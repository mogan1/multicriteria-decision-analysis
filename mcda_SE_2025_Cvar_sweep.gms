...
* We start after deriving the minimum and maximum value of Cost, emission, CvaR in their individual run of minimization
* Individual CVaR minimization resulted in an emission of 0 which is also the value for emission minimization and spurious cost.
* Hence a feasible region needs to be defined with the premise that during CVaR minimization there must be an
* increase of cost than cost minimization and increase of emission than emission minimization. So, epsilon-minimization
* of CVaR is incorporated with lower bound of emission kept at 5% of Emax and upper bound of cost kept at 25% of Cmin    
* 
Cmax  =4.12541E+09 ;   
Emin  =1e-2         ;     
Emax  =440714.77;
...
* Emin/Emax and Cmin/Cmax are defined in INCLUDE
    ...
    eps_em_min              'Minimal nonzero emission to avoid degenerate CVaR solutions' /1e-2/,
    cvar_bound              'used to apply dynamic CVaR caps'



;
* ------------------------------------------------------------
* DEFINE TIGHTER AND MORE PHYSICAL BOUNDS
* ------------------------------------------------------------
* Used in L165 & L166
Scalar
* +25% budget
    C_ub   
* >= 5% of typical emission
    E_lb     
;
*Should not allow to shoot more than 25% above min
    C_ub   = Cmin*1.25;
* >= 5% of typical emission
*Should not allow to shoot more than 5% above min
    E_lb   = Emax*0.05 ;  
alpha = 0.95;
...
* CVaR CAP GRID
* ------------------------------------------------------------
* 10 CVaR points from Qmin → Qmax
* We can increase the iteration more
Set qsteps /q1*q10/;    

Parameter qcap(qsteps) "CVaR caps for sweep";

Loop(qsteps,
    qcap(qsteps) = Qmin + (ord(qsteps)-1)/(card(qsteps)-1) * (Qmax - Qmin);
);
...
Equations
    ...
    emission_lb             'Introduced auxiliary constraint for CVar Run'
    cost_cap                'Cost ≤ C_ub'
    emission_lb             'Emission ≥ E_lb'
    cvar_cap_sweep;
...
*emission_lb..                                   of_emission =g= eps_em_min;

* New boundary imposed
cost_cap..                                      of_cost     =l= C_ub;
emission_lb..                                   of_emission =g= E_lb;
cvar_cap_sweep..                                of_cvar =l= cvar_bound;

* ------------------------------------------------------------
* BASE CVar MODEL
* ------------------------------------------------------------
* Add all thebounds in below model
model cvar_sweep_lp /
    cost_eq, emission_eq, cvar_eq,
    energy_balance, flow_pos_limit, flow_neg_limit, load_shed_limit,
    gen_capacity_limit, gen_avail_limit,
...
Parameter sweep_results(qsteps,*) "Cost/Emission/CVaR results";
* It is fine to use the .l instead of calculation again as we have now added cost_eq, emission_eq, and cvar_eq in the model itself

Loop(qsteps,
* set RHS dynamically
    cvar_bound = qcap(qsteps);
    solve cvar_sweep_lp using lp minimizing of_cvar;

    sweep_results(qsteps,"Cost")    = of_cost.l;
    sweep_results(qsteps,"Emission")= of_emission.l;
    sweep_results(qsteps,"CVaR")    = of_cvar.l;
...
file csv /'cvar_sweep_output_2025_12_11.csv'/; put csv;
put 'Step,CVARcap,Cost,Emission,CVAR' /;

loop(qsteps,
    put qsteps.tl, ',',
        qcap(qsteps):12:4, ',',
        sweep_results(qsteps,"Cost"):12:2, ',',
        sweep_results(qsteps,"Emission"):12:2, ',',
        sweep_results(qsteps,"CVaR"):12:2 /
);

putclose csv;

execute_unload 'cvar_sweep_output_2025_12_11.gdx', qcap, sweep_results;
